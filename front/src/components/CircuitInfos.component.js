...
export default class CircuitInfos extends Component {
    ...
    componentDidMount() {
        ...
        /*var circuitId = window.location.pathname.split("/")[2];
        ...
        // TODO Remplacer la valeur par l'ID du circuit
        this.getComment(11);
    }

    // RECUPERATION DU CIRCUIT
    /*async getCircuit(circuitId) {
        return await fetch('http://localhost:8080/api/poi/getCircuit/' + circuitId)
            .then(response => response.json())
            .then(data => {
                this.setState({circuitReady: true, circuit: data})
            });
    }*/
  ...
  render() {
    ...
    // TODO à remplacer par l'ID du circuit et non d'un POI
    const circuitId = "11";
    return (
        <div className="container">
            {(this.state.meteoReady && this.state.commentsReady) ?
            <div>
